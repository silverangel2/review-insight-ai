...
export async function collectAndAnalyzeReviewEvidence(
  ...
  const reviewCollectionAllowed =
    Boolean(listingUrlForReviewCollector) &&
    !listingRejectedForCollection;

  let exactListingAccepted = reviewCollectionAllowed;

  // Written-review evidence may only be accepted after the exact listing
  // verifier has allowed collection for this product.
  let collectorSourceAccepted = reviewCollectionAllowed;
  ...
  if (
    ...
    if (nativeReviewRetrieval.reviewsCollected > 0) {
      ...
      // Recovery may add review bodies, but it must never fabricate exact
      // listing verification or override a rejected collection state.
      collectorSourceAccepted =
        reviewCollectionAllowed &&
...
`;
  ...
  try {
    ...
    const configuredDeepSearchPasses = Number(
      process.env.REVIEWINTEL_DEEP_SEARCH_PASSES ?? 0
    );

    // Deep public web recovery is disabled by default. It can be explicitly
    // enabled with one pass, but can never run repeatedly.
    const maxDeepSearchPasses = Number.isFinite(configuredDeepSearchPasses)
      ? Math.max(0, Math.min(configuredDeepSearchPasses, 1))
      : 0;

    const collectedWrittenReviewSignalCount = Math.max(
      Number(collectedWrittenReviews.reviewsCollected ?? 0),
      Array.isArray(collectedWrittenReviews.reviews)
        ? collectedWrittenReviews.reviews.length
        : 0
    );
    ...
    for (
      ...
      reviewCollectionAllowed &&
      deepSearchPass <= maxDeepSearchPasses &&
      Math.max(
        parsedEvidenceSignalCount(parsed),
        collectedWrittenReviewSignalCount
...
`.trim();
    ...
    const preservedCollectorHasWrittenReviews =
      collectorHasWrittenReviews ||
      collectedWrittenReviews.collectorHasWrittenReviews === true ||
      Number(collectedWrittenReviews.reviewsCollected ?? 0) > 0 ||
      (Array.isArray(collectedWrittenReviews.reviews) &&
        collectedWrittenReviews.reviews.length > 0) ||
      preservedCollectedReviewCount > 0 ||
