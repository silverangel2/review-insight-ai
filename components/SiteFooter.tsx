  ...
  return (
    <footer className="border-t border-cyan-100/70 bg-white/85 px-6 py-10 text-ink shadow-[0_-20px_70px_rgba(8,183,168,0.06)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/90 dark:text-white">
          ...
          <div className="relative">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold uppercase text-cyan-100">AI review intelligence</p>
              </div>
            </div>
            ...
            <div className="mt-5 grid gap-2 text-sm text-cyan-50">
              <p>
                Support: <a className="font-black underline-offset-4 hover:underline" href={`mailto:${SUPPORT_EMAIL}`}>{SUPPORT_EMAIL}</a>
              </p>
              <p>
                Billing: <a className="font-black underline-offset-4 hover:underline" href={`mailto:${BILLING_EMAIL}`}>{BILLING_EMAIL}</a>
              </p>
            </div>
          </div>
        </section>
        ...
        <nav className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4" aria-label="Footer links">
          {footerLinkGroups.map((group) => (
            <section key={group.title} className="rounded-2xl border border-line bg-white p-5 shadow-soft dark:border-white/10 dark:bg-white/[0.04]">
              <h2 className="text-sm font-black uppercase text-slate-500 dark:text-slate-400">{group.title}</h2>
              <div className="mt-4 grid gap-3">
                {group.links.map((link) => (
                  <Link key={link.href} href={link.href} className="text-sm font-bold text-slate-600 transition hover:text-ocean dark:text-slate-300 dark:hover:text-cyan-200">
                    {link.label}
                  </Link>
                ))}
              </div>
      ...
      <div className="mx-auto mt-8 flex max-w-7xl flex-col gap-2 border-t border-line pt-5 text-xs font-bold uppercase tracking-wide text-slate-500 dark:border-white/10 dark:text-slate-400 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span>AI guidance for review decisions. Always verify critical product claims.</span>
      </div>
    </footer>
  );
}
