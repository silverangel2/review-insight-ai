...
export function OwnerAccessPanel() {
  ...
  async function openAccount(account: OwnerAccount) {
    ...
    const clientAccount: Parameters<typeof saveClientAccount>[0] = {
    ...
    let persistedAccount = getClientAccount();

    if (
      !persistedAccount ||
      persistedAccount.role !== account.role ||
      persistedAccount.plan !== account.plan
    ) {
      clearClientAccount({ preserveOwnerSession: true });
      saveClientAccount(clientAccount);
      persistedAccount = getClientAccount();
    }
    ...
    if (
      ...
      setError(`Could not switch to ${account.label}. The previous test account was not replaced.`);
      return;
    }
    ...
    window.location.replace(account.route);
  }
