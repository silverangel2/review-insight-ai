...
import { getUiTextTranslation, readStoredLocale, type ReviewIntelLocale } from "@/lib/i18n";
...
function translateResultItems(locale: ReviewIntelLocale, values: string[]) {
...
function shortShopperPhrase(value: string | undefined, fallback: string, maxWords = 6) {
  ...
  if (lower.includes("no strong repeated complaint") || lower.includes("no repeated complaint")) return "No repeated complaint";
  if (/\b(packaging|plastic|wrap|box|delivery|shipping)\b/.test(lower)) return "Packaging concern";
  if (/\b(fit|fits|size|sizing|compatible|compatibility|model)\b/.test(lower)) return "Fit or sizing concern";
  if (/\b(broke|broken|durable|durability|weak|stopped|leak|leaking)\b/.test(lower)) return "Durability concern";
  if (/\b(return|refund|support|seller|warranty|replacement)\b/.test(lower)) return "Support risk";
  if (/\b(price|value|worth|cost|budget)\b/.test(lower)) return "Price-value signal";
...
function buildTrustCriteria(result: AnalyzeResponse) {
  ...
  const criteriaSeeds = [
    ...
    },
    {
      ...
      why: "The value score weighs price clues, usefulness, material quality, expectation match, and overall buying signals.",
...
function TrustCriterionCard({ criterion }: { criterion: TrustCriterion }) {
  ...
  return (
    <article className={`min-h-[520px] rounded-[1.75rem] border ${style.border} bg-white p-5 shadow-soft dark:bg-slate-950`}>
      ...
      <div className="mt-3 grid gap-3">
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t("trustScore")}</p>
          </div>
      ...
      >
        {open ? t("hideProof") : t("seeSellerProof")}
      </button>
    </article>
  );
}
      ...
      <div className="rounded-[2rem] border border-line bg-[linear-gradient(135deg,#f8fbff,#eef7ff_42%,#fff7eb)] p-5 shadow-soft dark:border-white/10 dark:bg-slate-950 dark:bg-none">
        <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            ...
            <h2 className="mt-4 max-w-3xl text-3xl font-black leading-tight text-ink dark:text-white">{t("trustQuestion")}</h2>
          ...
          <div className="rounded-[1.75rem] border border-white/70 bg-white/80 p-5 shadow-soft backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.04]">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">{t("overallTrustScore")}</p>
      ...
      <article className="rounded-[2rem] border border-line bg-white p-6 shadow-soft dark:border-white/10 dark:bg-slate-950">
        <div className="grid gap-6 lg:grid-cols-[0.75fr_1.25fr]">
          <div>
            ...
            <h3 className="mt-3 text-3xl font-black break-words text-ink dark:text-white">{t("trustBlockerHeading")}</h3>
            <div className="mt-5 grid gap-3">
              ...
              <div className="rounded-2xl border border-line bg-mist p-4 dark:border-white/10 dark:bg-white/[0.04]">
                <p className="text-xs font-black uppercase text-slate-500 dark:text-slate-400">{t("biggestConversionOpportunity")}</p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-line p-5 dark:border-white/10">
              <p className="text-xs font-black uppercase text-slate-500 dark:text-slate-400">{t("topActionsNext")}</p>
            ...
            <div className="rounded-2xl border border-line p-5 dark:border-white/10">
              <p className="text-xs font-black uppercase text-slate-500 dark:text-slate-400">{t("listingImprovements")}</p>
...
function BuyerResults({ result }: { result: AnalyzeResponse }) {
  ...
  const sentimentPercentScore = analysis.sentiment_percentage ?? sentimentToPercent(analysis.sentiment_score);
  const positiveSignals = analysis.praised_features.length + analysis.positive_points.length + (analysis.value_score !== undefined && analysis.value_score >= 70 ? 1 : 0);
  const negativeSignals = analysis.common_complaints.length + analysis.negative_points.length + analysis.quality_concerns.length + analysis.durability_issues.length;
  const rawComplaintPressure = analysis.complaint_severity_score ?? clamp(analysis.common_complaints.length * 14 + analysis.negative_points.length * 8);
  const praiseRelief =
    positiveSignals >= Math.max(3, negativeSignals * 2) && sentimentPercentScore >= 68
      ? 18
      : positiveSignals > negativeSignals && sentimentPercentScore >= 60
        ? 10
        : 0;
  const complaintPressure = clamp(rawComplaintPressure - praiseRelief);
  const score = scoreNumber(analysis.product_score);
  const balancedVerdict: CustomerRecommendation =
    recommendation.verdict === "Avoid" && score >= 68 && fakeRisk < 65 && complaintPressure < 62 && positiveSignals > negativeSignals
      ? "Maybe"
      : recommendation.verdict === "Maybe" && score >= 82 && fakeRisk < 45 && complaintPressure < 45 && positiveSignals >= negativeSignals + 2
        ? "Buy"
        : recommendation.verdict;
  const verdictTone = recommendationTone(balancedVerdict);
  ...
  const verdictLabel = balancedVerdict.toUpperCase();
  ...
  const shopperRiskPhrase =
    meta.review_count_estimate < 10
      ...
      : complaintRatio > 0
        ...
        : "Most buyers liked the product.";
  const localizedVerdictLabel = translateResultText(locale, verdictLabel);
  const localizedBiggestComplaint = translateResultText(locale, biggestComplaint);
  const localizedBuyingNotesPhrase = translateResultText(locale, buyingNotesPhrase);
  const localizedShopperRiskPhrase = translateResultText(locale, shopperRiskPhrase);
  const localizedFakeRiskShort = translateResultText(locale, fakeRiskShort);
  const localizedRiskLevel = translateResultText(locale, riskLevel);
  const localizedValueVerdict = translateResultText(locale, valueVerdict);
  const verdictStyle = {
  ...
  const quickCards: ShopperCard[] = [
    {
      ...
      status: translateResultText(locale, `${fakeRiskShort} fake risk`),
      frontLines: translateResultItems(locale, [`Fake risk: ${fakeRiskShort}`, fakeRisk >= 65 ? "Pattern signals need checking." : fakeRisk >= 35 ? "Some wording feels patterned." : "Reviews look natural."]),
      details: translateResultItems(locale, uniqueSellerItems(
        [
          ...analysis.fake_review_indicators,
      ...
      checks: translateResultItems(locale, ["Look for verified use, photos, dates, and balanced praise/complaints."]),
      avoid: translateResultText(locale, fakeRisk >= 65 ? "You see repeated generic wording or suspicious review timing." : "You need very high proof before buying."),
      recommendation: translateResultText(locale, fakeRisk >= 65 ? "Check review authenticity first." : "Review signal is usable.")
    },
    {
      ...
      status: translateResultText(locale, "Promise match"),
      frontLines: translateResultItems(locale, [`Strength: ${mainStrength}`, `Complaint: ${biggestComplaint}`]),
      details: translateResultItems(locale, uniqueSellerItems(
      ...
      checks: translateResultItems(locale, ["Confirm size, material, compatibility, and the exact version you will receive."]),
      avoid: translateResultText(locale, productRealityScore < 45 ? "The product promise does not match repeated customer reality." : "The top complaint is a dealbreaker for you."),
      recommendation: translateResultText(locale, productRealityScore >= 70 ? "Product promise looks believable." : "Compare the promise against reviews.")
    },
    {
      title: "Risk",
      ...
      visual: "alert",
      icon: "!",
      metric: localizedRiskLevel,
      status: translateResultText(locale, `${safeRiskScore}% safety`),
      frontLines: translateResultItems(locale, [`Biggest issue: ${biggestComplaint}`, `Should buyer worry? ${buyerWorry}`]),
      details: translateResultItems(locale, uniqueSellerItems(
      ...
      checks: translateResultItems(locale, ["Check returns, warranty, seller support, shipping, and the repeated complaint."]),
      avoid: translateResultText(locale, riskScore >= 65 ? "The warning affects your main reason for buying." : "You cannot return it easily."),
      recommendation: translateResultText(locale, buyerRecommendationLine(balancedVerdict, riskLevel))
    }
  ];
        ...
        <div className="relative grid gap-5 lg:grid-cols-[0.72fr_1.28fr] lg:items-stretch">
          <div className="relative overflow-hidden rounded-[2rem] border border-white/70 bg-white/75 p-6 shadow-soft backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.05]">
            ...
            <div className="mt-8 flex items-end justify-between gap-4">
              <div>
                <h1 className="text-3xl font-black leading-none text-ink dark:text-white lg:text-3xl">{localizedVerdictLabel}</h1>
            ...
            <p className="mt-5 text-lg font-black leading-7 text-ink dark:text-white">{translateResultText(locale, buyerRecommendationLine(recommendation.verdict, riskLevel))}</p>
            <p className="mt-2 text-sm font-semibold leading-6 text-slate-600 dark:text-slate-300">{localizedShopperRiskPhrase}</p>
          </div>
          ...
          <div className="rounded-[2rem] border border-white/70 bg-white/70 p-5 shadow-soft backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.04]">
            <div className="grid gap-3 sm:grid-cols-2">
              {[
                [translateResultText(locale, "Fake Review Risk"), localizedFakeRiskShort, fakeRisk >= 65 ? "text-coral" : fakeRisk >= 35 ? "text-amber" : "text-teal"],
                [translateResultText(locale, "Value"), localizedValueVerdict, valueScore >= 72 ? "text-teal" : valueScore >= 48 ? "text-amber" : "text-coral"],
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {buildBuyerChecklist(analysis as unknown as Record<string, unknown>).map((item) => (
            <div key={item} className="rounded-2xl border border-line bg-slate-50 p-4 text-sm font-bold leading-6 text-ink dark:border-white/10 dark:bg-white/5 dark:text-white">
              {translateResultText(locale, item)}
            </div>
          ))}
        </div>
      </section>
...
function buildBuyerChecklist(analysis: Record<string, unknown>) {
  ...
  return [
    ...
    `Check AI-like review signs: ${fakeRisk}`,
    value,
...
function withDashboardSafeAnalysis(result: AnalyzeResponse): AnalyzeResponse {
  ...
  const positives = [
    ...safeStringArray(raw.positive_points),
    ...safeStringArray(raw.positivePoints),
    ...safeStringArray(raw.praised_features),
    ...safeStringArray(raw.praisedFeatures),
    ...safeStringArray(raw.strengths)
  ];

  const negatives = [
    ...safeStringArray(raw.common_complaints),
    ...safeStringArray(raw.negative_points),
    ...safeStringArray(raw.negativePoints),
    ...safeStringArray(raw.complaints),
    ...safeStringArray(raw.mainConcerns),
    ...safeStringArray(raw.topComplaints),
    ...safeStringArray(raw.pain_points),
    ...safeStringArray(raw.painPoints)
  ];

  const quality = safeStringArray(raw.quality_concerns);
  const durability = safeStringArray(raw.durability_issues);
  const support = safeStringArray(raw.support_issues);
  const improvements = safeStringArray(raw.improvement_suggestions);
  const packaging = safeStringArray(raw.packaging_issues);
  const fakeIndicators = safeStringArray(raw.fake_review_indicators);

  const sellerInsightsRaw = (raw.seller_insights || {}) as Record<string, unknown>;
  const sellerInsights = {
    main_customer_pain_points: safeStringArray(sellerInsightsRaw.main_customer_pain_points).length
      ? safeStringArray(sellerInsightsRaw.main_customer_pain_points)
      : negatives,
    product_improvement_recommendations: safeStringArray(sellerInsightsRaw.product_improvement_recommendations).length
      ? safeStringArray(sellerInsightsRaw.product_improvement_recommendations)
      : improvements,
    listing_improvement_suggestions: safeStringArray(sellerInsightsRaw.listing_improvement_suggestions),
    packaging_shipping_issues: safeStringArray(sellerInsightsRaw.packaging_shipping_issues).length
      ? safeStringArray(sellerInsightsRaw.packaging_shipping_issues)
      : packaging,
    refund_risk_issues: safeStringArray(sellerInsightsRaw.refund_risk_issues),
  ...
  return {
    ...
    meta: {
      ...result.meta,
      review_count_estimate: result.meta?.review_count_estimate ?? 0,
      confidence_label: result.meta?.confidence_label ?? "Low",
      audience: result.meta?.audience ?? "buyer"
    },
    analysis: {
      ...raw,

      overall_summary: raw.overall_summary || raw.summary || "Latest scan loaded.",
      ...
      praised_features: positives,
      praisedFeatures: positives,
      strengths: positives,

      common_complaints: negatives,
      negative_points: negatives,
      negativePoints: negatives,
      complaints: negatives,
      mainConcerns: negatives,
      topComplaints: negatives,
      pain_points: negatives,
      painPoints: negatives,

      quality_concerns: quality,
      product_quality_concerns: quality,
      durability_issues: durability,
      support_issues: support,
      improvement_suggestions: improvements,
      packaging_issues: packaging,
      fake_review_indicators: fakeIndicators,
      consistency_warnings: safeStringArray(raw.consistency_warnings),
      feature_requests: safeStringArray(raw.feature_requests),

      bestFor: safeStringArray(raw.bestFor),
      best_for: safeStringArray(raw.best_for),
      notIdealFor: safeStringArray(raw.notIdealFor),
      ...
      keyword_analysis: Array.isArray(raw.keyword_analysis) ? raw.keyword_analysis : [],
      keywords: Array.isArray(raw.keywords) ? raw.keywords : []
    } as unknown as AnalyzeResponse["analysis"]
  };
}
...
export function ResultsDashboard({ result, accountPlan }: { result: AnalyzeResponse; accountPlan?: SubscriptionPlan | null }) {
  ...
  if (safeResult.meta.audience === "seller" || safeResult.meta.audience === "both") {
    return <SellerResults result={safeResult} accountPlan={accountPlan} />;
  }

  return <BuyerResults result={safeResult} />;
}
