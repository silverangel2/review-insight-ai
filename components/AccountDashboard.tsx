...
const validPlans = new Set<SubscriptionPlan>(planOptions.map((option) => option.plan));
...
function roleLabel(role: ClientAccount["role"] | undefined) {
...
function publicAccountEmail(account: ClientAccount | null) {
  if (!account) return "";
  return isPrivateTestEmail(account.email) ? "Private owner access" : account.email;
}

function cleanTestValue(value: string | undefined | null) {
  if (!value) return "";
  const blocked = [
    "Seller Pro",
    "Seller Premium",
  ...
  return blocked.includes(value) ? "" : value;
}
...
export function AccountDashboard() {
  ...
  useEffect(() => {
    const requestedPlan = params.get("plan") as SubscriptionPlan | null;
  ...
  const activePlan = account?.plan ?? "free_buyer";
  const developerMode = isAdminRole(account?.role);
  const planTone = useMemo(() => (activePlan !== "free_buyer" ? "good" : "warn"), [activePlan]);
  const visiblePlanOptions = useMemo(() => {
    if (developerMode) return planOptions;
    if (account?.role === "seller") return planOptions.filter((option) => option.plan === "seller_starter" || option.plan === "seller_pro");
    return planOptions.filter((option) => option.plan === "free_buyer" || option.plan === "buyer_pro");
  }, [account?.role, developerMode]);
  ...
  async function refreshAccount() {
    ...
    if (data.account?.email) {
      ...
      const nextAccount: ClientAccount = {
        userId: serverTrusted ? (serverAccount.userId ?? existing?.userId ?? null) : (existing?.userId ?? serverAccount.userId ?? null),
        authUserId: serverTrusted ? (serverAccount.authUserId ?? existing?.authUserId ?? null) : (existing?.authUserId ?? serverAccount.authUserId ?? null),
        email: serverTrusted ? (serverAccount.email ?? existing?.email ?? "local@reviewintel.local") : (existing?.email ?? serverAccount.email ?? "local@reviewintel.local"),
        name: serverTrusted ? (serverAccount.name ?? existing?.name ?? "ReviewIntel user") : (existing?.name ?? serverAccount.name ?? "ReviewIntel user"),
        plan: serverTrusted ? (serverAccount.plan ?? existing?.plan ?? "free_buyer") : (existing?.plan ?? serverAccount.plan ?? "free_buyer"),
        role: serverTrusted ? (serverAccount.role ?? existing?.role ?? "buyer") : (existing?.role ?? serverAccount.role ?? "buyer"),
        ...
        subscriptionStatus: serverTrusted ? (serverAccount.subscriptionStatus ?? existing?.subscriptionStatus) : (existing?.subscriptionStatus ?? serverAccount.subscriptionStatus),
  ...
  async function choosePlan(plan: SubscriptionPlan) {
    ...
    if (plan !== "free_buyer") {
      ...
      if (data.mode === "local-demo" || data.mode === "developer-simulated") {
        setClientPlan(plan);
        setAccount(getClientAccount());
        setQuota(getStoredQuota());
      }
  ...
  function changePassword() {
    ...
    if (!account) return;
    const nextAccount = { ...account, passwordUpdatedAt: new Date().toISOString() };
    saveClientAccount(nextAccount);
    setAccount(nextAccount);
    setPasswordForm({ current: "", next: "", confirm: "" });
    setPasswordNotice("Password update saved for this QA account. Live accounts can also use the reset flow.");
  }
    ...
    <section className="space-y-8">
      ...
      <section className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <article className="rounded-2xl border border-line bg-white p-6 shadow-soft dark:border-white/10 dark:bg-slate-950">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Badge tone={account.role === "seller" ? "info" : "good"}>{roleLabel(account.role)} profile</Badge>
          </div>

          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {[
              ["name", "Display name", "Your name"],
              ["profileId", "Profile ID", "SHOP-001 or SELL-001"],
          ...
          <button onClick={saveProfile} className="mt-5 rounded-xl bg-ink px-5 py-3 text-sm font-black text-white transition hover:bg-ocean dark:bg-white dark:text-ink">
            Save profile
          </button>
        </article>
        ...
        <article className="rounded-2xl border border-line bg-white p-6 shadow-soft dark:border-white/10 dark:bg-slate-950">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
            Change the local QA password state here. Production password reset remains available from login.
          </p>
          ...
          {account.passwordUpdatedAt ? (
            <p className="mt-4 text-xs font-bold text-slate-500 dark:text-slate-400">Last password update: {new Date(account.passwordUpdatedAt).toLocaleString()}</p>
          ) : null}
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <button onClick={changePassword} className="rounded-xl bg-ink px-5 py-3 text-sm font-black text-white transition hover:bg-ocean dark:bg-white dark:text-ink">
              Update password
            </button>
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {visiblePlanOptions.map((option) => (
          <article key={option.plan} className="rounded-2xl border border-line bg-white p-6 shadow-soft dark:border-white/10 dark:bg-slate-950">
            ...
            >
              {option.plan === activePlan ? "Current plan" : developerMode ? "Open Admin QA" : `Use ${option.title}`}
            </button>
          </article>
        ))}
      </section>
          ...
          {recentAnalyses.length ? recentAnalyses.map((item) => (
            <article key={item.id} className="rounded-xl border border-line p-4 dark:border-white/10">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="font-black text-ink dark:text-white">{item.product_name || "Untitled product"}</p>
      ...
      <section className="grid gap-5 md:grid-cols-3">
        {[
          ...
          <Link key={href} href={href} className="rounded-2xl border border-line bg-white p-5 shadow-soft transition hover:-translate-y-0.5 hover:border-ocean dark:border-white/10 dark:bg-slate-950">
