...
export function sanitizeInsightList(items: Array<string | undefined>, fallback: string[] = [], limit = 6) {
  ...
  for (const item of [...items, ...fallback]) {
    const clean = compactRepeatedClauses(cleanReviewInsightText(item));
    if (!clean) continue;

    const key = normalizeKey(clean);
    if (!key || seen.has(key) || isNearDuplicateInsight(clean, output)) continue;
    seen.add(key);
...
export function sanitizeSellerInsightList(items: Array<string | undefined>, fallback: string[] = [], limit = 6) {
  ...
  for (const item of [...items, ...fallback]) {
    const theme = compactRepeatedClauses(sellerFriendlyTheme(item));
    if (!theme || theme === "not enough clean review evidence") continue;

    const key = normalizeKey(theme);
    if (!key || seen.has(key) || isNearDuplicateInsight(theme, output)) continue;
    seen.add(key);
...
function compactRepeatedClauses(value: string) {
  const clauses = value
    .split(/(?<=[.!?])\s+|;\s+|\s+\|\s+/)
    .map((part) => part.trim())
    .filter(Boolean);
...
function tokenSet(value: string) {
  return new Set(
    normalizeKey(value)
      .split(" ")
      .filter((token) => token.length > 3 && !["review", "reviews", "product", "seller", "buyer", "buyers", "customer", "customers"].includes(token))
  );
}
...
function isNearDuplicateInsight(value: string, existing: string[]) {
  return existing.some((item) => {
    if (normalizeKey(item) === normalizeKey(value)) return true;
    if (sellerFriendlyTheme(item) === sellerFriendlyTheme(value)) return true;
    return overlapRatio(item, value) >= 0.72;
  });
}
