...
export function normalizePlan(plan: string | null | undefined): SubscriptionPlan {
  if (plan === "buyer_pro" || plan === "pro") return "buyer_pro";
  if (plan === "seller_starter") return "seller_starter";
  if (plan === "seller_pro" || plan === "team") return "seller_pro";
  return "free_buyer";
}
...
export function planPrice(plan: SubscriptionPlan) {
  ...
  if (plan === "seller_starter") return "$29";
  if (plan === "seller_pro") return "$79";
  return "$0";
}
...
export function isSellerPlan(plan: SubscriptionPlan) {
  return plan === "seller_starter" || plan === "seller_pro";
}
...
export function canAccessSellerAnalytics(role: UserRole, plan: SubscriptionPlan) {
  return isAdminRole(role) || role === "seller" || isSellerPlan(plan);
}
