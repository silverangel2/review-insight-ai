...
import { supabaseUpdate } from "@/lib/supabaseServer";
...
export async function POST(request: Request): Promise<Response> {
  ...
  const subject = String(body.subject ?? "ReviewIntel Support").trim();
  ...
  if (!resendApiKey) {
    return NextResponse.json(
      { error: "RESEND_API_KEY is missing. Add it to Vercel or .env.local." },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    await resend.emails.send({
      from: "ReviewIntel Support <support@getreviewintel.com>",
      to,
...
${message}
                ...
                <div style="background:#f8fafc;border:1px solid #e6eaf2;border-radius:20px;padding:18px;margin-top:22px;">
                  <p style="margin:0;font-size:13px;line-height:1.7;color:#667085;">
                    Need to add more details? Reply to this email or contact us at ${SUPPORT_EMAIL}.
                  </p>
                </div>
              </div>
            </div>
