  ...
  useEffect(() => {
    ...
    async function completeGoogleAuth() {
      ...
      if (!params.accessToken) {
        setError(
          params.code
            ? "Google returned an authorization code, but this build is missing the token handoff. Check the Supabase OAuth flow setting."
            : "Google login did not return a session token. Please try again."
        );
        return;
      }
      ...
      saveActiveMode(account.role === "seller" ? "seller" : "buyer");
      clearLatestResult();
      setMessage("Google sign in complete. Opening your dashboard...");

      if (!cancelled) {
        router.replace(account.role === "seller" ? "/dashboard/seller" : "/dashboard/customer");
      }
    }
